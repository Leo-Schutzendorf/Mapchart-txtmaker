...
def parse_column_positions(table):
    ...
    col_labels = {}
    flat_col = 0
    # Use last header row that contains party/candidate names;
    # row[0] is often a title, row[-1] is often just # / %
    # So pick the row with the most useful label content
    label_row = header_rows[0]
    for i, row in enumerate(header_rows):
        texts = [th.get_text(strip=True) for th in row]
        if any(x in " ".join(texts) for x in ["Republican", "Whig", "Democrat", "Democratic", "DFL"]):
            label_row = row
            break
...
for state in states:
    ...
    for tr in table.find_all('tr'):
        ...
        # Before the 1980 election, Cibola County, NM was part of Valencia County.  Before the 1984 election, La Paz County, AZ was part of Yuma County.  Add these counties with the same results as their parent counties, but print a warning since this is an approximation.
