...
for state in states:
    ...
    #DC can be treated as a county-equivalent for MapChart, but its results are in an infobox, not a table.  I chose to do this after Delaware because it's next in ABC order.
    ...
    if state == "Delaware":
        ...
        if infobox:
            ...
            for row in rows:
                if 'District of Columbia' in row.get_text():
                    cells = row.find_all(['th', 'td'])
                    if len(cells) >= 3:
                        dem_pct = safe_float(cells[1].get_text())
                        rep_pct = safe_float(cells[2].get_text())
                        if rep_pct >= dem_pct:
                            winner, winner_pct = 'Republican', rep_pct
                        else:
                            winner, winner_pct = 'Democrat', dem_pct
